...
def _needs_binding_schema_reset() -> bool:
    with engine.connect() as conn:
        ...
        required = {"printer_id", "printer_ip", "printer_model", "adapter_name"}
...
def _needs_device_schema_reset() -> bool:
    with engine.connect() as conn:
        ...
        col_names = {str(col[1]) for col in cols}
        required = {"device_ip", "is_bound", "bound_printer_id", "detected_model", "detected_adapter", "probe_reachable"}
        return not required.issubset(col_names)
...
def init_db() -> None:
    if ORCH_DATABASE_URL.startswith("sqlite") and (_needs_binding_schema_reset() or _needs_device_schema_reset()):
        _reset_local_tables()
