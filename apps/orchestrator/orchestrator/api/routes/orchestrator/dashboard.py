...
<section class='card'><h3>Binding MAC/IP -> printer_id</h3><div class='row'><input id='printerId' placeholder='PRN-01'/><input id='printerMac' placeholder='02:42:ac:19:00:0a'/><input id='printerIp' placeholder='10.0.0.21 (optionnel)'/><select id='printerModel'><option value=''>Auto detect</option><option>CORE ONE</option><option>XL</option><option>MK4S</option><option>MK4</option><option>MK3.9S</option><option>MK3.5S</option><option>MINI+</option><option>SL1S</option><option>SL1</option><option>HT90</option></select><button onclick='bindPrinter()'>Bind</button></div><p id='bindMsg' class='muted'></p></section>
...
<section class='card'><h3>Fleet</h3><table id='fleetTable'><thead><tr><th>printer_id</th><th>mac</th><th>ip</th><th>modele</th><th>etat</th><th>heartbeat</th></tr></thead><tbody></tbody></table></section>
<section class='card'><h3>Devices non bindes</h3><table id='unboundTable'><thead><tr><th>mac</th><th>ip</th><th>etat</th><th>modele detecte</th><th>heartbeat</th></tr></thead><tbody></tbody></table></section>
</main><script>
...
function render(){
  document.querySelector('#fleetTable tbody').innerHTML=state.fleet.map(f=>`<tr><td>${fmt(f.printer_id)}</td><td>${fmt(f.printer_mac)}</td><td>${fmt(f.printer_ip)}</td><td>${fmt(f.printer_model)}</td><td>${fmt(f.status)}</td><td>${fmt(f.last_heartbeat_at)}</td></tr>`).join('');
  document.querySelector('#unboundTable tbody').innerHTML=state.unbound_ips.map(u=>`<tr><td>${fmt(u.device_mac)}</td><td>${fmt(u.device_ip)}</td><td>${fmt(u.status)}</td><td>${fmt(u.detected_model)}</td><td>${fmt(u.last_heartbeat_at)}</td></tr>`).join('');
}
async function bindPrinter(){const pid=document.getElementById('printerId').value.trim();const ip=document.getElementById('printerIp').value.trim();const mac=document.getElementById('printerMac').value.trim();const model=document.getElementById('printerModel').value.trim();const msg=document.getElementById('bindMsg');const body={printer_id:pid,printer_model:model||null};if(ip)body.printer_ip=ip;if(mac)body.printer_mac=mac;try{await api('/printer-bindings',{method:'POST',body:JSON.stringify(body)});msg.textContent='OK';}catch(e){msg.textContent=e.message}}
