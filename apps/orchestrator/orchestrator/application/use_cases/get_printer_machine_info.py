...
class GetPrinterMachineInfoUseCase:
    ...
    def execute(self, printer_id: str) -> dict:
        ...
        payload = adapter.get_machine_info(printer_ip)
        payload["printer_id"] = printer_id
        payload["binding"] = {
            "printer_ip": printer_ip,
            "printer_mac": binding.printer_mac,
            "printer_serial": snapshot.get("device_serial") if snapshot else None,
            "printer_model": snapshot.get("detected_model") if snapshot else None,
